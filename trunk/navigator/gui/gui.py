...
class Gui(QtGui.QWidget):
    ...
    def __init__(self, opt):
        ...
        self.pathErrorcurve = self.pathErrorplot.plot(self.pathErrordata, pen=(0,0,0),
            name="Path Error")
        if self.opt == 'sim' :
            self.poseErrordata =np.zeros(250) 
            self.poseErrorcurve = self.pathErrorplot.plot(self.poseErrordata, pen=(0,0,150),
                name="Position Error")
        self.ptr = 0      
...
class GLWidget(QtOpenGL.QGLWidget):
    ...
    def __init__(self, opt, path, parent=None):
        ...
        self.opt = opt
        self.pose3d = None
        self.trailbuff = RingBuffer(400)
        if self.opt == 'sim' :
            self.simtrailbuff = RingBuffer(400)
        self.path = path
        self.viewpoint = True
        self.view_d = 20.0
